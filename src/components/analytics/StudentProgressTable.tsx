...
import { Users, CheckCircle, AlertTriangle, XCircle, ExternalLink } from 'lucide-react';
...
export function StudentProgressTable({ students }: StudentProgressTableProps) {
  ...
  const { role } = useAuth();
  const canViewDetails = role === 'admin' || role === 'teacher';
            ...
            <TableBody>
              {sortedStudents.map((student) => {
                ...
                return (
                  <TableRow 
                    key={student.userId}
                    className={canViewDetails ? 'cursor-pointer hover:bg-secondary/50 group' : ''}
